...
async def criar_categoria(categoria: Categoria):
    ...
    if len(categoria.nome) < 3 or len(categoria.nome) > 50:
        raise HTTPException(status_code=422, detail="O nome da categoria deve ter entre 3 e 50 caracteres")
...
async def excluir_categoria(categoria_id: int):
    ...
    return {"message": "Categoria e suas memórias associadas foram excluídas com sucesso"}
